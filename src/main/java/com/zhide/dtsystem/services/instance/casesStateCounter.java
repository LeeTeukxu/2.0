...
public class casesStateCounter {
    ...
    public Map<String, Integer> getResult(Integer userId, String roleName) {
        ...
        List<IStateCounter> all = Arrays.asList(
                ...
                new Five(dataSource, userId, roleName),
                new Six(dataSource, userId, roleName),
                new Seven(dataSource, userId, roleName),
            ...
            if (FindCounter.isPresent()) {
                ...
                if (roleName.equals("系统管理员") || roleName.equals("财务人员")) {
                    ...
                    if (X < 9 && X != 7) {
                        ...
                        finds = dataSource.stream().filter(f -> f.getState() < 8).collect(toList());
                    }
