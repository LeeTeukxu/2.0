...
@Controller
@RequestMapping("/work/casesConfirm")
public class CasesConfirmController {
    @Autowired
    ...
    @Autowired
    ICasesConfirmBrowseService confirmService;
    @Autowired
    ...
    public pageObject getData(HttpServletRequest request) {
        ...
        try {
            result = confirmService.getData(request);
        } catch (Exception ax) {
