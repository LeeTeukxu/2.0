...
<@layout>
    ...
    <script type="text/javascript">
        ...
        // var gx='<script type="text/javascript" src="/js/common/complexExcelExport.js?v='+Date.now()+'">'+"<\/script>";
        // document.write(gx);
    </script>
    <style type="text/css">
        .info1top ul {
        ...
        <div region="center" style="overflow:hidden">
            <div class="mini-toolbar">
                                ...
                                <div id="info1" style="height:55px;background:rgb(226,250,252);border-radius: 5px;border:1px solid rgb(190,226,240);">
                                    <div class="info1top" style="padding-left: 120px;">
                                        ...
                                        <ul>
                                            ...
                                            <li class="Jdlcli AppWatchlySource clicked" id="J2" >
                                            ...
                                            <li class="Jdlcli AppWatchlySource" id="J4" >
                                                <a style="text-decoration:none" target="_self" href="#"
                                                   onclick="changeQuery('JKStatus',3,this)">
                ...
                <table style="width:100%">
                    <tr>
                        <td style="width:95%">
                            <a class="mini-button mini-button-danger" id="FeeItem_NoPayMark"
                               onclick="changePayStatus('0')">标记不缴费</a>
                            <a class="mini-button mini-button-success" id="FeeItem_PayMark" onclick="changePayStatus
                            ('1')">标记需缴费</a>
                            <a class="mini-button mini-button-info" id="FeeItem_AddToFeeList"
                               onclick="addToPayForWait">添加到缴费清单</a>
                            <span class="separator FeeItem_Query"></span>
                            <a class="mini-button mini-button-info" id="FeeItem_Export" onclick="doExport">导出Excel</a>
                            <a class="mini-button mini-button-info" id="FeeItem_ExportFee" onclick="doExportYear">导出代缴费清单</a>
                            <a class="mini-button mini-button-primary" id="FeeItem_Jiaodui" onclick="Jiaodui">校对金额</a>
                            <a class="mini-button mini-button-primary" id="FeeItem_SXF" onclick="addSXMoney">手续费</a>
                            <span class="separator FeeItem_Query"></span>
                            <button class="mini-button FeeItem_Query" iconCls="icon-reload"
                        ...
                        <td style="white-space:nowrap;">
                            <div class="mini-combobox Query_Field FeeItem_Query"  id="comField" style="width:100px"
                                 data="[{id:'All',text:'全部属性'},{id:'ShenQingh',text:'专利号'},{id:'FeeName',text:'费用名称'},
                                 {id:'FAMINGMC',text:'发明名称'},{id:'SHENQINGRXM',text:'申请人姓名'},{id:'ANJIANYWZT',text:'专利状态'},{id:'FAMINGRXM',text:'发明人姓名'},{id:'KH',text:'所属客户'},{id:'XS',text:'销售人员'},{id:'DL',text:'代理师'},{id:'LC',text:'流程人员'},{id:'CustomMemo',text:'手动标识'}]" value="All"
                                 id="Field"></div>
                            <input class="mini-textbox Query_Field FeeItem_Query" style="width:150px" id="QueryText" />
                            <a class="mini-button mini-button-success" onclick="doQuery();" id="FeeItem_Query">模糊搜索</a>
                            <a class="mini-button mini-button-danger" id="FeeItem_Reset" onclick="reset">重置</a>
                            <a class="mini-button" id="FeeItem_HighQuery" iconCls="panel-expand" onclick="expand">高级查询</a>
                        </td>
                    </tr>
                </table>
                <div id="p1" style="border:1px solid #909aa6;border-top:0;height:150px;padding:5px;display:none">
                    <table style="width:100%;" id="highQueryForm">
                        <tr>
                            ...
                            <td style="width:15%;"><input name="FeeName" class="mini-textbox" data-oper="LIKE" style="width:100%"/></td>
            ...
            <div class="mini-fit" id="fitt">
                <div id="FeeItem_datagrid" class="mini-datagrid" style="width: 100%; height: 100%;" onrowclick="onRowClick"
                     ...
                     ondrawcell="onDraw" allowresize="true" url="/watch/applyList/getData" multiselect="true"
                     pagesize="20"
                     sizelist="[5,10,20,50,100,200,500,1000,1500,2000]" autoload="false" onload="afterload">
                    <div property="columns">
                        ...
                        <div field="JkStatus" align="center" name="JkStatus"  width="90" headerAlign="center" type="comboboxcolumn" allowsort="true">监控状态
                            <div property="editor" class="mini-combobox" data="ApplyWatch"></div>
                        </div>
                        ...
                        <div field="MONEY" name="money" width="90" allowsort="true" headeralign="center" align="right">
                            金额
                        </div>
...
<script type="text/javascript">
    ...
    var NoPayMark = mini.get('FeeItem_NoPayMark');
    var ParyMark = mini.get('FeeItem_PayMark');
    var ToPayForWait = mini.get('FeeItem_AddToFeeList');
    $(function(){
        $('#p1').hide();
        fit.setHeight('100%');
        fit.doLayout();
    ...
    function addToPayForWait() {
        ...
        mini.open({
            url:'/watch/addToPayForWait/index?IDS='+ids.join(',')+'&Mode=Add&Type=Apply',
            width:800,
            height:410,
            showModal:true,
            title:'添加到缴费清单',
    ...
    function onDraw(e) {
        ...
        if (field == "status") {
            ...
            var record = e.record;
            var ApplyWatch = record["ApplyWatch"];
        ...
        mini.prompt("人工校对金额：", "请输入",
                function (action, value) {
                    if (action == "ok") {
                        ...
                        var url = '/watch/applyList/changeJiaoDuiMoney';
    ...
    function addSXMoney(){
        ...
        function (action, value) {
            if (action == "ok") {
                ...
                var url = '/watch/applyList/changeShouXiuMoney';
                $.post(url, {IDS:mini.encode(ids),Money:value }, function (r) {
                    if (r['success']) {
    ...
    function changePayStatus(rr){
        ...
        var msg="确认要将选择的<"+rows.length+">条记录"+(rr=='0'?'标记为【不缴费】？':'标记为【缴费】?');
        mini.confirm(msg,'设置提示',function(action){
            if(action=='ok'){
                var url = '/watch/applyList/changePayStatus';
                $.post(url, {IDS:mini.encode(ids),Status:rr}, function (r) {
                    ...
                    else mini.alert(r['message'] || "缴费状态修改失败!");
                });
            }
        });
    }
    ...
    function updateNumber() {
        var url="/watch/applyList/refreshTotal";
    ...
    function ShowMemo(id, title) {
        ...
        mini.open({
            url: '/watch/feeWatch/addMemo?Type=Apply&ID=' + id,
            showModal: true,
            width: 1000,
            height: 600,
            title: "【" + title + "】备注信息",
