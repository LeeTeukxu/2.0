...
<@layout>
    ...
    <div class="mini-fit">
        <div class="mini-datagrid" id="datagrid1" style="width:100%;height:100%" allowCellEdit="true" allowCellSelect="true"
             allowCellValid="true" url="/work/noticePeriodConfig/getData" autoload="true" pageSize="20"
             sortField="TypeName" sortOrder="asc">
            <div property="columns">
                <div type="indexcolumn"></div>
                <div field="typeName" width="200" headerAlign="center" vtype="required">
                    通知书类型
        ...
        if (row) {
            ...
            if (id) {
                mini.confirm('确认要删除选择的设置', '删除提示', function (btn) {
                    if (btn == 'ok') {
                        var url = '/work/noticePeriodConfig/remove';
    ...
    function SaveConfig() {
        ...
        if (grid.isValid()) {
            function p() {
                ...
                var url = '/work/noticePeriodConfig/save';
            ...
            mini.confirm('确认要保存设置数据吗？', '保存提示', function (btn) {
                if (btn == 'ok') p();
            });
        } else {
