    ...
    <div class="baseinfo">
        <div style="width: 100%; overflow: hidden; margin-top: 10px">
            <h1>销售回款信息管理<span style="color: red; font-size: 14px; font-family: 仿宋"></span>
                <span style="margin-right: 0px; float: right; font-size: 16px"></span>
            </h1>
        </div>
    </div>
                ...
                <div id="Info1" class="mini-panel mini-panel-info" title="2、商务人员填写" width="auto"
                    ...
                    <div id="Table2">
                        ...
                        <div class="mini-datagrid" id="gridDetail" style="width:100%;height:300px" id="grid1"
                             ...
                             showSummaryRow="true" allowCellSelect="true" allowCellEdit="false" sortField="moneyType"
                             oncellbeginedit="onBeforeEditMain" onshowrowdetail="onShowRowDetail" onload="afterload"
                             url="/arrivalUse/getDetail?ArrID=${ArrivalRegistrationID}" autoload="true"
                             ondrawsummarycell="drawSummary">
    ...
    <script type="text/javascript">
        ...
        function saveRow() {
            ...
            for (var i = 0; i < rows.length; i++) {
                ...
                if (total > max) {
                    mini.alert("第" + (i + 1) + "行的数据超过了允许最大录入金额!");
                    return;
                }
                totalMoney += total;
                rs.push(row);
            ...
            if (field == "Action") {
                ...
                if((fs||"").toString().length>0){
                    ...
                    var x = '<a href="javascript:void(0)" style="color:blue;text-decoration:underline"' +
                        ' onclick="uploadRow(' + "'" + record._id + "','"+ "Browse" + "')" + '">'+'&nbsp;' + uText +
                        '&nbsp;' + '</a>';
                    e.cellHtml = x;
                }
            }
                ...
                if (form.isValid()) {
                    ...
                    $.post(url, arg,
                        function (text) {
                            ...
                            if (res.success) {
                                var data = res.data || {};
                                mini.alert('销售回款信息提交审核成功!', '系统提示', function () {
                                    form.setData(data);
                                    if (window["CloseOwnerWindow"]) window.CloseOwnerWindow();
                                });
                            } else {
        ...
        function getRen() {
            ...
            for (var i = 0; i < rows.length; i++) {
                var row = rows[i];
                var money = parseFloat(row["money"]);
                var max = parseFloat(row["max"]);
                if (money > max) {
                    throw "第" + (i + 1) + '行数据录入的金额超过了允许录入的最大值。';
                }
                var state = parseInt(row["state"] || 0);
                if (state == 1) {
                    continue;
            ...
            if (totalMoney > allMoney) throw "累计认领的金额超过了到帐金额!";
