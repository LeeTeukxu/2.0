(function ($) {
    $.fn.fileUploader = function (options) {
        var fileGrid = null, addButton = null, removeButton = null, uploadButton = null, downButton = null, configButton = null,saveButton = null,exportButton = null, configCombobox = null, uploader = null;
        var ds = this;
        var buttons = [
        ];
        var listResult={};
        var container = $(this), defaults = {
            ...
            ///保存数据地址
            saveUrl:'/common/excelImport/saveProductItemType',
            ///删除文件地址
            removeUrl: '/SimpleFlow/UploadService.aspx?Method=RemoveFile',
            ///设置附件类型
        ...
        function createGrid() {
            var op = [
                ...
                '<div field="Name" name="Name" width="150" headeralign="center" >产品名称</div>',
                '<div field="Type" name="Type" width="80" headeralign="center" >产品分类</div>',
                '<div field="Cost" name="Cost" width="100" headeralign="center">产品成本</div>',
                '<div field="Price" name="Price" width="100" headeralign="center" >产品报价</div>',
                '<div field="Required" name="Required" width="80" headerAlign="center" >业务交单要求</div>',
                '<div field="Memo" name="Memo" width="150" headeralign="center" >备注</div>',
                '</div>',
                '</div>',
                '</div>',
            ...
            uploader = new plupload.Uploader({
                ...
                init: {
                    ...
                    FileUploaded: function (up, file, res) {
                        var AllName=[];
                        AllName=AllProductItemType.split(',');
                        var files = fileGrid.getData();
                        ...
                        if (result['success']) {
                            // if (saveButton) saveButton.show();
                                ...
                                for (var i = 0; i < files.length; i++) {
                                    ...
                                    //字段非空判断
                                    if (data.data[i].Name == "") {
                                        ErrorResult+="产品名称不能为空,";
                                    }  if (data.data[i].Type == "") {
                                        ErrorResult+="产品类型不能为空,";
                                    }  if (data.data[i].Cost == "") {
                                        ErrorResult+="产品成本不能为空,";
                                    }  if (data.data[i].Price == "") {
                                        ErrorResult+="产品报价不能为空,";
                                    }  if (data.data[i].Required == "" ) {
                                        ErrorResult+="业务交单要求不能为空,";
...
//弹出隐藏层
        ...
        function load(t = 0) {
            ...
            if (t <= TMAX) {
                ...
                document.getElementById("ClientList").style.filter="blur(3px)";
            }
            //加载完成之后关闭
            if(document.getElementById("p").value==100){
                document.getElementById("MyDiv").style.display='none';
                document.getElementById("fade").style.display='none';
                document.getElementById("ClientList").style.filter="none";
            }
            ...
            function bindToolbar() {
                ...
                saveButton.on('click',function () {
                    ...
                    mini.confirm('确认要保存上传的数据?','系统提示',function (btn) {
                        if (btn=='ok'){
                            $.post(settings.saveUrl,{Data:mini.encode(listResult)},function (r,c) {
                                var res=mini.decode(r);
                                if (res['success']){
                                    mini.alert('保存数据成功！');
